...
export default function AIWaiter() {
  const { subdomain, branch, branchSlug } = useParams<{
    subdomain?: string; branch?: string; branchSlug?: string;
  }>();
  ...
  const startLevelMeter = (analyser: AnalyserNode) => {
    ...
    const tick = () => {
      ...
      // scale & smooth to feel responsive but not jittery
  ...
  async function startListening() {
    try {
      ...
      analyser.fftSize = 1024; // decent resolution
      ...
      ws.onopen = () => {
        ...
        ws.send(JSON.stringify({ t: 'hello', sessionId: 'dev-session', userId: 'guest', rate: 16000, ch: 1, lang: 'auto' }));
        setListening(true);
      };
  ...
  const activeChip =
    'bg-[#FA2851] text-white border-[#FA2851] shadow-[0_6px_20px_rgba(250,40,81,0.25)]';
  const inactiveChip =
    'bg-white/80 text-gray-700 border-white/70 backdrop-blur hover:bg-white';
  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-col items-center justify-center">
        {/* MicHalo now uses `mode` instead of `listening` */}
        <MicHalo
          size={600}
          color={haloColor}
          opacity={0.6}
          accentColor={ACCENT}
          mode={haloMode as any}
          level={level}
        />
        ...
        {/* See Menu — white & classy */}
        <Link
          to={seeMenuHref}
          className="
            mt-6 z-[3]
            px-6 py-3 rounded-full text-[16px] font-medium
            bg-white/95 text-[#FA2851] 
            border border-white/80 shadow-[0_4px_14px_rgba(0,0,0,0.06)]
            backdrop-blur-xl
