...
function jsonProxyToAuth() {
  return createProxyMiddleware({
    ...
    onProxyReq: (proxyReq: ClientRequest, req: Request & { body?: unknown }) => {
      ...
      const ct =
        (proxyReq.getHeader('content-type') as string | undefined) || '';
    ...
    onError(err: NodeErr, req: Request, res: Response) {
      ...
      if (!res.headersSent) {
        res
          .status(502)
          .json({
            message: 'Bad gateway (auth-service unavailable)',
            code,
          });
      }
    },
...
app.use('/api/v1/public', publicProxyToAuth());
...
const VOICE_WS_TARGET =
  process.env.VOICE_WS_TARGET || 'http://ai-waiter-service:7071';

const voiceWsProxy = createProxyMiddleware({
  target: VOICE_WS_TARGET,
  changeOrigin: true,
  ws: true,
  logLevel: 'debug', // temporary for debugging
  ...
  onError(err: Error, req: any) {
    logger.error(
      `[WS][VOICE] Proxy error: ${req?.url} -> ${VOICE_WS_TARGET} :: ${err.message}`,
    );
  },
  onProxyReqWs(_proxyReq: any, req: any, _socket: any, options: any) {
    const targetStr =
      options?.target?.toString?.() || VOICE_WS_TARGET || 'UNKNOWN_TARGET';
    logger.info(
      `[WS][VOICE] Upgrading ${req?.url} -> ${targetStr}`,
    );
  },
});
...
if (process.env.NODE_ENV !== 'production') {
  app.use('/api', (req: Request, res: Response) => {
    ...
    res
      .status(404)
      .json({ message: 'Route not found', path: req.originalUrl });
  });
}
